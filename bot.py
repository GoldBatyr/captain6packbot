...
QUESTIONS = [
    ...
    },
    {
        ...
        "ru_options": ["A) Всеми судами при встрече/пересечении/обгоне в видимости", "B) Всеми судами в пределах полумили вне видимости", "C) Моторными: обгон в видимости / пересечение в полумиле и в видимости", "D) Моторными при пересечении в полумиле вне видимости"],
        "correct": 2,
    ...
    },
    {
        ...
        "ru_options": ["A) Круговой белый ИЛИ жёлтый", "B) Только круговой жёлтый", "C) Только круговой белый", "D) Только белый 225 градусов"],
        "correct": 0,
...
def send_glossary(chat_id, context, index, message_to_delete=None):
    term = GLOSSARY[index]
    caption = f"Глоссарий {index + 1} из {len(GLOSSARY)}\n\nEN: {term['term']}\nRU: {term['ru']}"
    prev_index = (index - 1) % len(GLOSSARY)
    keyboard = [
        [InlineKeyboardButton("<< Назад", callback_data=f"glo{prev_index}")],
...
def button(update: Update, context: CallbackContext):
    ...
    if query.data == "menu_quiz":
        ...
        keyboard = [[InlineKeyboardButton("Следующий / Next >>", callback_data="next_question")]]
