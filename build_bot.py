...
def button(update, context):
    ...
    if query.data in ("main_menu", "main_menu_from_glo"):
        ...
        if total_answered >= 8 and not db_is_paid(user_id):
            ...
            return
        state["g_index"] = 0
        intro = "🔄 База постоянно пополняется. Покупатели пакетов за $249 и $499 получают новые термины и все обновления бота раньше всех в автоматическом режиме.\n\nBase is growing. $249 and $499 members get updates first."
        try:
